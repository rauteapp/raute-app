...
export default function LoginPage() {
    ...
    // Initialize from URL query parameters (which only happens safely on client)
    useEffect(() => {
        ...
        if (error) {
            ...
            setRedirectError(errorMessages[error] || 'Something went wrong. Please try signing in again.')
        }

        const msg = params.get('message')
        if (msg === 'verified') setMessage('✅ Email verified! Please sign in to continue.')
        else if (msg === 'try_login') setMessage('Please sign in with your credentials to continue.')
    ...
    return (
        <MobileAuthWrapper
            mobileChildren={
                    ...
                    <div className="flex flex-col h-[100dvh] w-full relative overflow-hidden bg-[#f8fafc]">
                        {/* Background — Abstract modern pattern or map */}
                        <div className="absolute top-0 left-0 w-full h-[55vh] pointer-events-none">
                            <img
                                src="/login-hero-map.png"
                                alt="Route Map"
                                className="w-full h-full object-cover object-top opacity-[0.35] mix-blend-multiply drop-shadow-sm"
                            />
                            <div className="absolute inset-0 bg-gradient-to-b from-[#f8fafc]/10 via-[#f8fafc]/60 to-[#f8fafc]"></div>
                        </div>

                        {/* Content overlay */}
                        <div className="relative z-10 flex flex-col h-full w-full overflow-y-auto overscroll-none scrollbar-hide">
                            {/* Header / Logo with Safe Area */}
                            <div className="pt-[max(env(safe-area-inset-top),24px)] px-6 flex-shrink-0 flex justify-between items-center">
                                <img src="/logo.png" alt="Raute" className="h-[28px] w-auto object-contain drop-shadow-sm" />
                            </div>

                            {/* Spacer for scrollable content */}
                            <div className="flex-1 shrink-0 min-h-[40px]" />

                            <div className="px-5 pb-[max(env(safe-area-inset-bottom),24px)] w-full max-w-[440px] mx-auto flex flex-col justify-end transition-all duration-300">
                                {/* Title Area optionally */}
                                <div className="mb-6 px-1 text-center sm:text-left">
                                    <h1 className="text-3xl font-bold tracking-tight text-slate-900 mb-1.5 drop-shadow-sm">
                                        {!showSignupForm ? "Welcome back" : "Create account"}
                                    </h1>
                                    <p className="text-[15px] text-slate-500 font-medium">
                                        {!showSignupForm ? "Log in to your Raute account" : "Start your free trial today"}
                                    </p>
                                </div>
                                ...
                                {!showSignupForm ? (
                                    <>
                                        ...
                                        <div className="bg-white/90 backdrop-blur-xl rounded-[24px] p-5 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.08)] border border-white/50 space-y-4 relative overflow-hidden flex flex-col">
                                            {/* decorative top glow */}
                                            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-indigo-500 opacity-90" />
                                            
                                            <form onSubmit={onSubmit} className="space-y-3.5 mt-1">
                                                {apiError && (
                                                    <div className="bg-red-50 text-red-600 text-[13px] font-medium p-3 rounded-xl flex items-start gap-2.5 border border-red-100/50">
                                                ...
                                                {redirectError && (
                                                    <div className="bg-red-50 text-red-600 text-[13px] font-medium p-3 rounded-xl flex items-start gap-2.5 border border-red-100/50">
                                                        <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" />
                                                        <span className="leading-tight">{redirectError}</span>
                                                    </div>
                                                )}
                                                {message && (
                                                    <div className="bg-emerald-50 text-emerald-600 text-[13px] font-medium p-3 rounded-xl flex items-start gap-2.5 border border-emerald-100/50">
                                                ...
                                                <div>
                                                    <Input
                                                        ...
                                                        className="h-[52px] bg-slate-50 border-transparent hover:border-slate-200 rounded-xl focus-visible:ring-blue-600 focus-visible:border-transparent text-[15px] px-4 shadow-none transition-all"
                                                        disabled={isLoading}
                                                        required
                                                    />
                                                </div>
                                                ...
                                                <div className="relative">
                                                    <Input
                                                        ...
                                                        className="pr-12 h-[52px] bg-slate-50 border-transparent hover:border-slate-200 rounded-xl focus-visible:ring-blue-600 focus-visible:border-transparent text-[15px] px-4 shadow-none transition-all"
                                                        disabled={isLoading}
                                                        required
                                                    />
                                                    <Button
                                                        ...
                                                        variant="ghost"
                                                        size="sm"
                                                        className="absolute right-1 top-1 h-[44px] w-[44px] text-slate-400 hover:text-slate-600 rounded-lg"
                                                ...
                                                <Button className="w-full h-[52px] mt-1 text-[15px] font-semibold rounded-xl bg-[#2563eb] hover:bg-[#1d4ed8] text-white shadow-[0_4px_12px_rgba(37,99,235,0.2)] hover:shadow-[0_6px_16px_rgba(37,99,235,0.3)] transition-all" type="submit" disabled={isLoading}>
                                                    {isLoading ? (
                                            ...
                                            <div className="flex items-center gap-3 py-1">
                                                ...
                                                <span className="text-[12px] font-semibold text-slate-400 uppercase tracking-wider">or continue with</span>
                                                <div className="flex-1 h-[1px] bg-slate-200/60"></div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                ...
                                                <button
                                                    ...
                                                    disabled={isLoading}
                                                    className="w-full h-[52px] bg-white rounded-xl flex items-center justify-center gap-2.5 border border-slate-200/80 hover:bg-slate-50 active:scale-[0.98] transition-all shadow-sm"
                                    ...
                                    <>
                                        {/* Signup Form — inline */}
                                        <div className="bg-white/90 backdrop-blur-xl rounded-[24px] p-5 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.08)] border border-white/50 relative overflow-hidden flex flex-col max-h-[75vh]" >
                                            {/* decorative top glow */}
                                            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-indigo-500 opacity-90" />
                                            
                                            <div className="overflow-y-auto overscroll-contain pr-1 -mr-1 space-y-3.5 scrollbar-hide pt-1 pb-2">
                                                <form onSubmit={onSignupSubmit} className="space-y-3.5">
                                                    {signupError && (
                                                        <div className="bg-red-50 text-red-600 text-[13px] font-medium p-3 rounded-xl flex items-start gap-2.5 border border-red-100/50 mb-2">
                                                            <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" />
                                                    ...
                                                    <div className="grid gap-3 grid-cols-2">
                                                        <div>
                                                            <Input
                                                                ...
                                                                type="text"
                                                                className="h-[52px] bg-slate-50 border-transparent hover:border-slate-200 rounded-xl focus-visible:ring-blue-600 focus-visible:border-transparent text-[15px] px-3.5 shadow-none transition-all"
                                                                disabled={isLoading}
                                                                required
                                                            />
                                                        </div>
                                                    ...
                                                    <div>
                                                        <StyledPhoneInput
                                                            ...
                                                            className="h-[52px] bg-slate-50 border-transparent rounded-xl hover:border-slate-200 shadow-none focus-within:ring-2 focus-within:ring-blue-600 overflow-hidden"
                                                            required
                                                        />
                                                    </div>
                                                    ...
                                                    <Button className="w-full h-[52px] mt-2 text-[15px] font-semibold rounded-xl bg-[#2563eb] hover:bg-[#1d4ed8] text-white shadow-[0_4px_12px_rgba(37,99,235,0.2)] hover:shadow-[0_6px_16px_rgba(37,99,235,0.3)] transition-all" type="submit" disabled={isLoading}>
                                                        {isLoading ? (
                                                ...
                                                <div className="flex items-center justify-center pt-4 mt-2 border-t border-slate-100">
                                                    <button
                                                        ...
                                                        className="text-[14px] font-semibold text-slate-600 hover:text-slate-900 flex items-center gap-1.5 transition-colors"
                                                    >
                                ...
                                <div className="mt-5 text-center px-4">
                                    <p className="text-[12px] text-slate-400/80 leading-relaxed font-medium">
                                        ...
                                        {" "}and{" "}
                                        <Link href="/terms" className="text-slate-500 hover:text-slate-700 underline underline-offset-2">terms of use</Link>
                                    </p>
                                </div>
                            </div>
                        </div>
